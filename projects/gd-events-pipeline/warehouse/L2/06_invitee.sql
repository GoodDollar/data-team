-- L2: invitee (VIEW)
-- Sources: Semantic.invite_signups, Semantic.invite_payouts, Semantic.claim_events
-- Purpose: per-invitee lifecycle entity. For each invitee, joins signup → post-signup
--          claims → payout → post-payout retention counts.
-- Grain:   1 row per invitee_address. Scoped to signup_type IN ('referral', 'campaign').
-- See docs/02_DATA_MODEL.md §Semantic.invitee

CREATE OR REPLACE VIEW `gooddollar.Semantic.invitee` AS
...
--    Dedup guard: if the contract has a bug and emits two payouts for the same invitee,
--    keep the earliest (EC9).
invitee_payouts AS (
  SELECT
    invitee_address,
...
--    after bounty payment. Single scan of claim_events with conditional aggregation.
--    Only populated for paid invitees (invitee_payouts is the driving table).
...
SELECT
  ...
  -- IMPORTANT: plain COALESCE alone would incorrectly convert no-payout rows to 0.
  -- The outer CASE WHEN is required (spec FC11).
