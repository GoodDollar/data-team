...
The builder loads its dropdown values from [`config.json`](config.json) at runtime. To add, rename, or remove a tag value:

1. Edit `config.json` — add or remove objects in the relevant array (`sources`, `mediums`, `initiatives`, `goals`). Each object needs a `value` (the raw UTM string, lowercase) and a `label` (display name in the dropdown).
2. Update the corresponding table in this document so the written standard stays in sync.
3. Open a PR — the live tool updates as soon as it merges to master.

No build step. The tool is plain HTML + JavaScript with zero dependencies.
