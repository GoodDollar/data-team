...
const WORKER_URL = 'https://gooddollar-antseed-integration.gooddollar.workers.dev';
    ...
    if (source === 'live') {
        if (liveAvailable) {
            ...
            document.getElementById('charts-section').querySelector('h2').insertAdjacentHTML('afterend',
                ...
                '<div class="emoji">&#128679;</div>' +
                '<p>Worker endpoint not yet deployed. Waiting on <a href="https://github.com/GoodDollar/antseed-integration/pull/19" target="_blank" rel="noopener">PR #19</a> to merge.</p>' +
                '</div>');
            document.getElementById('table-content').innerHTML =
                '<div class="empty-state">' +
                '<div class="emoji">&#128679;</div>' +
                '<p>Live data will appear here once the Worker endpoint is deployed.</p>' +
                '</div>';
            ...
            document.getElementById('last-updated-text').textContent = 'Endpoint not yet deployed -- auto-retries every 5 min';
            return;
        }
    } else {
        analyticsData = demoData;
